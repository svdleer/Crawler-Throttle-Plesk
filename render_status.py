...
def rows(items: list[dict], columns: list[tuple[str, str]]) -> str:
    if not items:
        return "<tr><td colspan='%d' class='empty'>Geen resultaten in dit venster.</td></tr>" % len(columns)
    rendered = []
    for item in items:
        cells = []
        for key, _ in columns:
            if key == 'remove':
                cells.append(f"<td><button class='remove' onclick=\"act('remove','{esc(item.get('ip', ''))}')\">Verwijder</button></td>")
                continue
            if key == 'ip':
