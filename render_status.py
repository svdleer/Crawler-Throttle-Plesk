...
def main() -> None:
    if len(sys.argv) not in {3, 4, 5}:
        raise SystemExit("usage: render_status.py LATEST OUTPUT [SUMMARY] [SERVER]")
    ...
    # Iris-specific branding and server name
    server_name = "Iris" if len(sys.argv) == 5 and "iris" in sys.argv[4].lower() else "Digisteden"
    brand_name = "Maxigraphx" if server_name == "Iris" else "Avant"
    base_path = "/crawl-throttle" if server_name == "Iris" else "/cfstatus"
    logo_url = "https://maxigraphx.nl/images/2024/09/11/logo_mgx.svg" if server_name == "Iris" else f"{base_path}/avant-webdiensten.svg"
    page_title = f"{brand_name} · Crawler Security"
    header_title = f"{brand_name} Crawler Throttle"
    attribution = "" if server_name != "Iris" else f"<div style='margin-top:8px;opacity:0.7;font-size:11px'>Powered by <img src='{base_path}/avant-webdiensten.svg' alt='Avant' style='height:14px;vertical-align:middle;filter:brightness(0) invert(1)'></div>"
    ...
    body = controls + "".join((
        panel("Active temporary blocks", "Local control-plane status; nothing is enforced at Cloudflare during dry-run.", active, [("ip", "IP"), ("remove", "") , ("level", "Level"), ("until", "Until"), ("owner", "Owner"), ("reason", "Reason"), ("status", "Status")]),
        panel("Permanent blocks", "Only proposals are shown during dry-run; permanent enforcement is not automatically enabled.", permanent, [("ip", "IP"), ("level", "Level"), ("owner", "Owner"), ("reason", "Reason"), ("status", "Status")]),
        panel("Recidivism nominations", "Nominations tracked by the local escalation ladder.", nominations, [("ip", "IP"), ("level", "Level"), ("until", "Until"), ("owner", "Owner"), ("reason", "Reason")]),
        panel("Recent policy transitions", f"24 hours: {counts.get('temporary_started', 0)} temporary starts · {counts.get('temporary_expired', 0)} expired · {counts.get('permanent_proposed', 0)} permanent proposals.", transitions, [("at", "Time"), ("description", "Transition"), ("ip", "IP"), ("level", "Level"), ("reason", "Reason")]),
        panel("Top 10 dynamic URLs · last hour", "Rolling overview from the lightweight incremental monitor.", top_urls, [("url", "URL"), ("requests", "Requests")]),
        panel("Detected patterns", "Slow periodic crawlers and other detections.", patterns, [("ip", "IP-adres"), ("reason", "Patroon"), ("avg_interval_seconds", "Gem. interval"), ("matching_intervals", "Passende intervallen"), ("unique_urls", "Unieke URLs")]),
    ))
    target.write_text(f"""<!doctype html><html lang='en'><head><meta charset='utf-8'><meta name='viewport' content='width=device-width,initial-scale=1'><title>{page_title}</title><style>:root{{--navy:#091d33;--blue:#0e497b;--cyan:#31d4e7;--lime:#b8dc45;--paper:#f3f6fa;--ink:#102234;--muted:#667789;--line:#dce5ed}}*{{box-sizing:border-box}}body{{margin:0;background:var(--paper);color:var(--ink);font:15px/1.45 Inter,ui-sans-serif,system-ui,-apple-system,sans-serif}}.top{{background:linear-gradient(115deg,var(--navy),var(--blue));color:#fff;padding:30px max(24px,calc((100vw - 1280px)/2));border-bottom:4px solid var(--cyan)}}.brand{{display:flex;align-items:center;gap:10px}}.mark{{max-width:220px;max-height:60px;object-fit:contain;object-position:left center}}.eyebrow{{margin:0;color:var(--cyan);font-size:11px;font-weight:800;letter-spacing:.16em}}h1{{margin:1px 0 0;font-size:26px;letter-spacing:-.03em}}.meta{{display:flex;gap:10px;flex-wrap:wrap;margin-top:21px}}.badge{{padding:6px 10px;border:1px solid #ffffff33;border-radius:99px;background:#ffffff12;font-size:12px}}.layout{{max-width:1280px;margin:25px auto;padding:0 24px 40px}}.grid{{display:grid;grid-template-columns:repeat(4,1fr);gap:14px;margin-bottom:18px}}.metric{{background:#fff;border-radius:12px;padding:16px;border:1px solid var(--line)}}.metric span{{color:var(--muted);font-size:12px;font-weight:700;text-transform:uppercase;letter-spacing:.08em}}.metric strong{{display:block;font-size:27px;margin-top:4px}}.panel{{background:#fff;border:1px solid var(--line);border-radius:12px;margin:15px 0;overflow:hidden}}.panel-head{{padding:16px 18px;display:flex;justify-content:space-between;gap:15px;align-items:center;border-bottom:1px solid var(--line)}}h2{{font-size:16px;margin:0}}p{{margin:3px 0 0;color:var(--muted);font-size:13px}}.count{{background:#e9f8fb;color:#076577;padding:3px 9px;border-radius:99px;font-weight:800}}.table-wrap{{overflow:auto}}table{{width:100%;border-collapse:collapse;min-width:600px}}th,td{{padding:10px 14px;text-align:left;border-bottom:1px solid #edf1f5;font-size:13px}}th{{background:#f8fafc;color:var(--muted);font-size:11px;text-transform:uppercase;letter-spacing:.07em}}.empty{{color:var(--muted);text-align:center;padding:20px}}#events{{margin:0;padding:16px 18px;background:#081522;color:#b9e7ee;max-height:360px;overflow:auto;font:12px/1.55 ui-monospace,SFMono-Regular,Menlo,monospace;white-space:pre-wrap}}.pulse{{display:inline-block;width:8px;height:8px;background:var(--lime);border-radius:50%;margin-right:6px;box-shadow:0 0 0 4px #b8dc4533}}.refresh{{height:4px;background:#ffffff26;position:relative;margin-top:20px;overflow:hidden;border-radius:99px}}.refresh i{{display:block;height:100%;width:100%;background:linear-gradient(90deg,var(--cyan),var(--lime));transform-origin:left center}}.refresh-label{{margin-top:7px;color:#d5e7f4;font-size:12px}}.ip-info{{position:relative;display:inline-block;color:#0e497b;font-weight:800;cursor:help}}.ip-info:hover::after{{content:attr(data-tooltip);position:absolute;z-index:20;left:50%;top:150%;transform:translateX(-50%);width:300px;padding:9px 11px;border-radius:6px;background:#091d33;color:#fff;font:12px/1.35 system-ui,sans-serif;text-align:left;box-shadow:0 5px 16px #0005}}button{{background:#0e497b;color:#fff;border:0;border-radius:6px;padding:9px 12px;cursor:pointer;font-weight:700}}button.remove{{background:#b42318;padding:6px 9px;font-size:12px}}button:hover{{filter:brightness(1.08)}}@media(max-width:900px){{.grid{{grid-template-columns:repeat(2,1fr)}}}}@media(max-width:760px){{.grid{{grid-template-columns:1fr}}.top{{padding:24px}}.layout{{padding:0 12px}}}}</style></head><body><header class='top'><div class='brand'><img class='mark' src='{logo_url}' alt='{brand_name}'><div><p class='eyebrow'>{brand_name.upper()} · SECURITY OPERATIONS</p><h1>{header_title}</h1>{attribution}</div></div><div class='meta'><span class='badge'><span class='pulse'></span>Local control plane active</span><span class='badge'>Mode: {esc(mode)}</span><span class='badge'>Log window: {esc(report.get('window_seconds', '?'))} seconds</span><span class='badge'>Updated: {generated}</span></div><div class='refresh'><i id='refresh-bar'></i></div><div class='refresh-label' id='refresh-label'>Next monitor refresh in 60 seconds</div></header><main class='layout'><div class='grid'><div class='metric'><span>Requests/s</span><strong id='rps'>—</strong></div><div class='metric'><span>Active workers</span><strong id='conns'>—</strong></div><div class='metric'><span>Recidivism nominations</span><strong>{len(nominations)}</strong></div><div class='metric'><span>Temporary Nginx 429 active</span><strong>{len(active)}</strong></div><div class='metric'><span>Permanent proposals</span><strong>{len(permanent)}</strong></div><div class='metric'><span>Transitions · 24 hours</span><strong>{sum(counts.values())}</strong></div></div>{body}<section class='panel'><div class='panel-head'><div><h2>Live policy event feed</h2><p>Refreshes every 5 seconds; shows readable local control-plane decisions.</p></div><span class='count'>STREAM</span></div><pre id='events'>Loading event feed…</pre></section></main><script>async function updateStats(){{try{{const r=await fetch('/nginx_status',{{cache:'no-store'}});const t=await r.text();if(t.includes('Active connections')){{const active=t.match(/Active connections: (\\d+)/);const accepts=t.match(/\\s+(\\d+)\\s+(\\d+)\\s+(\\d+)/);if(active)document.getElementById('conns').textContent=active[1];if(accepts){{const reqs=parseInt(accepts[3]);const stored=parseInt(localStorage.getItem('nginx_reqs')||reqs);const elapsed=(Date.now()-(parseInt(localStorage.getItem('nginx_ts')||Date.now())))/1000;if(elapsed>1){{const rps=Math.round((reqs-stored)/elapsed);document.getElementById('rps').textContent=rps;localStorage.setItem('nginx_reqs',reqs);localStorage.setItem('nginx_ts',Date.now())}}}}}}else{{const lines=t.split('\\n');const data={{}};lines.forEach(l=>{{const m=l.match(/^(\\w+):\\s+(.+)$/);if(m)data[m[1]]=m[2]}});if(data.ReqPerSec)document.getElementById('rps').textContent=parseFloat(data.ReqPerSec).toFixed(1);if(data.BusyWorkers&&data.IdleWorkers)document.getElementById('conns').textContent=parseInt(data.BusyWorkers)+parseInt(data.IdleWorkers)}}}}catch(e){{}}}}function readable(e){{const level=e.level||'—',reason=e.reason?' · '+e.reason:'';if(e.type==='php_fpm_restart')return'PHP-FPM 8.4 restart completed · load '+e.load+' · active workers '+e.active+' · Digisteden pool '+e.digisteden_pool+'/'+e.digisteden_pool;if(e.type==='php_fpm_overload_sample')return'PHP-FPM overload sample · load '+e.load+' · samples '+e.samples+'/3';if(e.type==='temporary_403')return'Temporary Nginx 429 scheduled: '+e.ip+' · level '+level+' · '+((e.duration_seconds||0)/60)+' min'+reason;if(e.type==='temporary_expired')return'Temporary Nginx 429 expired: '+e.ip+' · level '+level;if(e.type==='permanent_block')return'Permanent block proposed: '+e.ip+' · level 4'+reason;if(e.type==='temporary_removed')return'Temporary block removed: '+e.ip;return'Unknown policy transition'}}async function poll(){{try{{const r=await fetch('{base_path}/events?ts='+Date.now(),{{cache:'no-store'}});const t=await r.text();document.getElementById('events').textContent=t.split('\\n').filter(Boolean).slice(-150).reverse().map(line=>{{try{{const e=JSON.parse(line),at=new Date(e.at).toLocaleString('en-US',{{hour:'2-digit',minute:'2-digit',second:'2-digit'}});return at+' · '+readable(e)}}catch(_){{return line}}}}).join('\\n')||'No policy events yet.'}}catch(e){{document.getElementById('events').textContent='Event feed unavailable: '+e}}}}async function setMode(tooling,dry_run){{const out=document.getElementById('control-result');out.textContent='Updating…';try{{const r=await fetch('{base_path}/action/mode',{{method:'POST',headers:{{'Content-Type':'application/json'}},body:JSON.stringify({{tooling,dry_run}})}});const d=await r.json();out.textContent=d.ok?'Mode updated':'Error: '+d.error;setTimeout(()=>location.reload(),800)}}catch(e){{out.textContent='Error: '+e}}}}async function act(kind,selected){{const ip=selected||document.getElementById('control-ip').value.trim(),out=document.getElementById('control-result');if(!ip){{out.textContent='Enter an IP address';return}}out.textContent='Working…';try{{const r=await fetch('{base_path}/action/'+kind,{{method:'POST',headers:{{'Content-Type':'application/json'}},body:JSON.stringify({{ip}})}});const d=await r.json();out.textContent=d.ok?'Success: '+ip:'Error: '+d.error;setTimeout(()=>location.reload(),800)}}catch(e){{out.textContent='Error: '+e}}}}updateStats();setInterval(updateStats,3000);poll();setInterval(poll,5000);const refreshSeconds=60,started=Date.now();function countdown(){{const elapsed=(Date.now()-started)/1000,remaining=Math.max(0,refreshSeconds-(elapsed%refreshSeconds));document.getElementById('refresh-bar').style.transform='scaleX('+remaining/refreshSeconds+')';document.getElementById('refresh-label').textContent='Next monitor refresh in '+Math.ceil(remaining)+' seconds'}}countdown();setInterval(countdown,250)</script></body></html>""", encoding="utf-8")
