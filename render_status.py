...
def main() -> None:
    ...
    body = controls + "".join((
        panel("Active temporary blocks", "Lokale control-plane status; in dry-run wordt niets bij Cloudflare afgedwongen.", active, [("ip", "IP"), ("remove", "") , ("level", "Niveau"), ("until", "Tot"), ("owner", "Owner"), ("reason", "Reden"), ("status", "Status")]),
        panel("Permanent blocks", "Alleen voorstellen in dry-run; permanente handhaving is niet automatisch ingeschakeld.", permanent, [("ip", "IP"), ("level", "Niveau"), ("owner", "Owner"), ("reason", "Reden"), ("status", "Status")]),
        panel("Recidivism nominations", "Nominaties die lokaal in de escalatieladder worden gevolgd.", nominations, [("ip", "IP"), ("level", "Niveau"), ("until", "Tot"), ("owner", "Owner"), ("reason", "Reden")]),
        panel("Recent policy transitions", f"24 uur: {counts.get('temporary_started', 0)} tijdelijk gestart · {counts.get('temporary_expired', 0)} verlopen · {counts.get('permanent_proposed', 0)} permanent voorgesteld.", transitions, [("at", "Tijd"), ("description", "Transitie"), ("ip", "IP"), ("level", "Niveau"), ("reason", "Reden")]),
