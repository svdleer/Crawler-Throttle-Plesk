...
def main() -> None:
    ...
    controls = f"""<section class='panel'><div class='panel-head'><div><h2>Enforcement control</h2><p><strong>{mode_label}</strong> · Office-only server-side control.</p></div></div><div style='padding:16px;display:flex;gap:8px;flex-wrap:wrap'><button onclick=\"setMode('on',false)\">Enable tooling</button><button onclick=\"setMode('off',false)\">Disable tooling</button><button onclick=\"setMode('on',true)\">Enable dry-run</button><button onclick=\"setMode('on',false)\">Disable dry-run</button><input id='control-ip' placeholder='IP-adres' style='padding:10px;min-width:200px'><button onclick=\"act('urgent')\">Urgent tijdelijke actie</button><button onclick=\"act('remove')\">Verwijder tijdelijke actie</button><span id='control-result'></span></div></section>"""
    body = controls + "".join((
