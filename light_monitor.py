...
def main():
 c=env(sys.argv[1]); state_p,latest_p,slow_p=map(Path,sys.argv[2:5]); log=Path(c['ACCESS_LOG']); now=datetime.now(timezone.utc); ns=nets(c['TRUSTED_LISTS']); dyn=tuple(c['DYNAMIC_PREFIXES'].split(',')); exc=tuple(c['EXCLUDED_PREFIXES'].split(',')); st=json.loads(state_p.read_text()) if state_p.exists() else {'inode':None,'offset':None,'ips':{}}
 info=log.stat(); inode=str(info.st_ino)
 if st['inode']!=inode or st['offset'] is None: st['inode'],st['offset']=inode,info.st_size; state_p.write_text(json.dumps(st)); latest_p.write_text(json.dumps({'mode':c.get('MODE','dry-run'),'window_seconds':60,'local_candidates':[],'block_candidates':[],'pattern_alerts':[]})); slow_p.write_text(json.dumps({'slow_candidates':[]})); return
 with log.open('rb') as f:f.seek(st['offset']);data=f.read(1048576);st['offset']=f.tell()
 cutoff=(now-timedelta(hours=24)).timestamp(); recent=(now-timedelta(seconds=60)).timestamp(); top_urls=Counter()
 for line in data.decode(errors='replace').splitlines():
  q=line.split('"');h=q[0].split() if len(q)>1 else [];req=q[1].split() if len(q)>1 else []
  if len(h)<5 or len(req)<2:continue
  try:ip=ipaddress.ip_address(h[0]);t=datetime.strptime(h[3].lstrip('[')+' '+h[4].rstrip(']'),FMT).timestamp()
  except:continue
 ...
 for ip,ev in list(st['ips'].items()):
  ...
  if len(one)>=15 and len(urls)>=5:candidates.append({'ip':ip,'requests':len(one),'unique_urls':len(urls),'429_events':0})
